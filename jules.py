    ...
    def parse_and_execute(self, code):
        ...
        while i < len(lines):
            ...
            # Parse repeat loops
                ...
                if 'times' in line:
                    # repeat n times
                    count = int(self._parse_expression(line.split('repeat')[1].split('times')[0].strip()))
                    done_idx = self._find_matching_done(lines, i)
    ...
    def _find_otherwise(self, lines, start_idx, end_idx):
        ...
        for i in range(start_idx + 1, end_idx):
            ...
            if line.startswith('when'):
                ...
                depth -= 1
            elif line == 'otherwise' and depth == 1:
                return i
            elif line.startswith('otherwise when') and depth == 1:
                return i
        return None
    ...
    def _import_library(self, lib_name):
        ...
        if lib_name == 'drawing':
            try:
                ...
                spec = importlib.util.spec_from_file_location("drawing", os.path.join("libs", "drawing.py"))
